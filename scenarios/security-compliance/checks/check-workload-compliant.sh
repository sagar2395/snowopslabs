...
# Grades the remediation half of the drill: the workload's Deployment must satisfy
...
if ! kubectl -n "$NS" get deployment "$DEPLOY" >/dev/null 2>&1; then
  ...
  echo "      Deploy it: labctl app build ${WORKLOAD_NAME} && labctl app deploy ${WORKLOAD_NAME}" >&2
  exit 1
fi
