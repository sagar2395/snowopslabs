...
if [ -n "$STUCK" ]; then
  echo "FAIL: go-api pod(s) cannot pull their image:" >&2
  echo "$STUCK" >&2
  echo "The replacement node has an empty image store. Re-import the lab images:" >&2
  echo "  k3d image import go-api:v1.2.0 -c \${CLUSTER_NAME:-snowops}" >&2
  exit 1
fi

NOT_RUNNING="$(echo "$PODS" | awk '$2 != "Running" {print $1" ("$2")"}')"
if [ -n "$NOT_RUNNING" ]; then
  echo "FAIL: go-api pod(s) are not Running:" >&2
  echo "$NOT_RUNNING" >&2
  exit 1
fi
...
if [ "${NODES:-0}" -lt 2 ]; then
  echo "FAIL: all go-api pods are on a single node (${NODES} node(s) in use)." >&2
  ...
  kubectl -n "$APP_NAMESPACE" get pods -l app=go-api -o wide >&2
  exit 1
fi

echo "OK: go-api is Running across ${NODES} nodes."
