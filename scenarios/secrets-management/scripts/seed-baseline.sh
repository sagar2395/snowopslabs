...
ES="${WORKLOAD_NAME:-go-api}-secret"
...
echo "Waiting for External Secrets to sync the baseline into ${NS}/${WORKLOAD_NAME:-go-api}-secrets..."
deadline=$(($(date +%s) + 120))
while [ "$(date +%s)" -lt "$deadline" ]; do
  got="$(kubectl -n "$NS" get secret "${WORKLOAD_NAME:-go-api}-secrets" \
...
echo "  Vault holds: $(kubectl exec -n vault vault-0 -- sh -c \
  "VAULT_ADDR=http://127.0.0.1:8200 VAULT_TOKEN='${ROOT_TOKEN}' vault kv get -field=api-key secret/${WORKLOAD_NAME:-go-api}" 2>/dev/null || echo '<unreadable>')" >&2
