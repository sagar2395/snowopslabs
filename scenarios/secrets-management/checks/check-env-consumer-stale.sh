...
pod_for() {
  kubectl -n go-api get pod -l "app=$1" -o jsonpath='{.items[0].metadata.name}' 2>/dev/null || true
}

FILE_POD="$(pod_for secret-consumer)"
ENV_POD="$(pod_for env-consumer)"
if [ -z "$FILE_POD" ] || [ -z "$ENV_POD" ]; then
  echo "NOT COMPLETE: expected both secret-consumer and env-consumer pods in namespace go-api." >&2
  exit 1
fi
...
if [ "$WANT" = "$BASELINE" ]; then
  echo "PENDING: secret/go-api is still the baseline ('${BASELINE}') — rotate it first." >&2
  exit 1
fi

FILE_VALUE="$(kubectl -n go-api exec "$FILE_POD" -- cat /etc/api/api-key 2>/dev/null || true)"
ENV_VALUE="$(kubectl -n go-api exec "$ENV_POD" -- sh -c 'echo "$API_KEY"' 2>/dev/null || true)"
