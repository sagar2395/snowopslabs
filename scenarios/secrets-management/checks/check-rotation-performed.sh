...
if [ "$CURRENT" = "$BASELINE" ]; then
  echo "PENDING: secret/${WORKLOAD_NAME:-go-api} is still the baseline ('${BASELINE}') — you have not rotated it yet." >&2
  exit 1
fi

echo "OK: secret/${WORKLOAD_NAME:-go-api} was rotated away from the baseline."
