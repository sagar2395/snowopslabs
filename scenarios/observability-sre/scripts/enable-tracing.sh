...
# Point ${WORKLOAD_NAME:-go-api} at the Alloy collector. This is the wiring step the collector
...
if ! kubectl get deployment "$DEPLOYMENT" -n "$NAMESPACE" >/dev/null 2>&1; then
  ...
  echo "Deploy the app first: labctl app deploy ${WORKLOAD_NAME:-go-api}" >&2
  exit 1
fi
