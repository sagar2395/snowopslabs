...
func executeOne(a Action, d Deps) error {
	...
	case "app-deploy":
		return d.Exec.RunScript("src/engine/deploy.sh", "deploy", a.Target)
	case "app-destroy":
		return d.Exec.RunScript("src/engine/deploy.sh", "destroy", a.Target)
	case "scenario-up":
	...
	case "traffic-stop":
		_, err := d.Exec.RunScriptStreamed("Stop traffic", "src/services/traffic/stop.sh")
		return err
	default:
