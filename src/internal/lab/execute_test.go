...
func fakeLabProject(t *testing.T) string {
	...
	// src/engine/deploy.sh records its calls (invoked with CWD = content root)
	os.MkdirAll(filepath.Join(root, "src", "engine"), 0755)
	os.WriteFile(filepath.Join(root, "src", "engine", "deploy.sh"),
		[]byte("#!/usr/bin/env bash\necho \"$1 $2\" >> deploy.log\n"), 0755)

	// src/services/traffic/stop.sh records its call
	os.MkdirAll(filepath.Join(root, "src", "services", "traffic"), 0755)
	os.WriteFile(filepath.Join(root, "src", "services", "traffic", "stop.sh"),
