...
# Scan the whole repository, not just this module: the portability gate covers
# every shell script in the tree (bootstrap/, platform/, runtimes/ at the
# content root as well as src/engine/ and src/services/). The script lives at
# src/scripts/, so the content root is two levels up; prefer the git top-level
# when available so the sweep is correct regardless of where it is invoked from.
...
collect() {
  find . \
    \( -path ./.git -o -path ./node_modules -o -path ./src/ui/node_modules \
